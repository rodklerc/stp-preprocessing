...
AllTransferPatternsCalculator.prototype._write = function (clusteringResult, encoding, done) {
  ...
  var aggregator = new TransferPatternGraphAggregator(clusteringResult.inverseClustering, clusteringResult.borderStations)
            ...
            function (minutes, seriesCallback) {
               ...
               var streamer = fetcher.buildConnectionsStream({"departureTime": dep},
                function (connectionsStream) {
                  console.log('CSA for ' + minutes);
                  var streamBuilderPromises = [];
                  var streams = [];
                  tasks.push.apply(tasks,
                    _und.map(self._clusteringResult.inverseClustering, // for every stop
                             function (cluster, stop) {
                               return function (callback) {
                                 var sbDeferred = Q.defer();
                                 streamBuilderPromises.push(sbDeferred.promise);
                                 ...
                                 var resultStream = planner.pipe(localPathBuilder);
                                 streams.push(planner);
                   ...
                   Q.all(streamBuilderPromises).then(function () {
                     connectionsStream.on('data', function (connection) {
                       for (var i in streams) {
                         var connectionCloned = {};
                         connectionCloned.departureStop = connection.departureStop;
                         connectionCloned.arrivalStop = connection.arrivalStop;
                         connectionCloned.departureTime = connection.departureTime;
                         connectionCloned.arrivalTime = connection.arrivalTime;
                         connectionCloned["@id"] = connection["@id"];
                         connectionCloned["@type"] = connection["@type"];
                         connectionCloned["gtfs:trip"] = {};
                         connectionCloned["gtfs:trip"]["@id"] = connection["gtfs:trip"]["@id"];

                         // We currently do not use the gtfs:route.
                         // Note that some servers will not provide a gtfs:route,
                         // uncommenting the next two lines will cause the
                         // preprocessing to stop working in that case.
                         /*connectionCloned["gtfs:route"] = {};
                         connectionCloned["gtfs:route"]["@id"] = connection["gtfs:route"]["@id"];*/

                         //connectionCloned.type = connection.type;
                         streams[i].write(connectionCloned);
                       }
                     });
