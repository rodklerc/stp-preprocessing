...
/**
 * A ClusterAlgorithm is a duplex stream. The input should be the output of
 * GraphBuilder. The output of this stream is an object with the following keys:
 * clustering: mapping of cluster ids to stops in the clustering
 * inverseClustering: mapping of stop id to cluster id
 * graph: the graph as produced by GraphBuilder
 * existingClusters: mapping of cluster id to boolean, indicating if the clustering
 * id is still in use.
 * borderStations: mapping of station to boolean, indicating wether the station
 * is classified as a border station.
...
ClusterAlgorithm.prototype._write = function (graphBuilderOutput, encoding, done) {
  this._graph = graphBuilderOutput.graph;
  this._tripToStations = graphBuilderOutput.tripToStations;
  done();
}
...
ClusterAlgorithm.prototype._determineBorderStations = function () {
  var self = this;
  ...
  for (var trip in this._tripToStations) {
    var uniqueClustersOnTrip = _und.uniq(_und.map(this._tripToStations[trip],
                                         function (station) {
                                           return self._inverseClustering[station];
                                         }));
    if (uniqueClustersOnTrip.length > 1) {
      for (var stop in this._tripToStations[trip]) {
        this._borderStations[this._tripToStations[trip][stop]] = true;
      }
    }
  }
}
...
ClusterAlgorithm.prototype._flush = function (callback) {
  ...
  // STEP 3: determine border stations
  this._determineBorderStations();
  console.log(this._borderStations);
