...
DAGBuilder.prototype._write = function (tp, encoding, done) {
  ...
  for (var i = 0; i < path.length - 1; i++) {
    if (this._pathExists(path[i+1], path[i])) {
      if (this._edgeExists(path[i], path[i+1])) {
        if (this._dag[path[i]][path[i+1]].length === 0) {
          for (var stop in this._stopsUsed) {
            if (this._dag[path[i]][path[i+1]].indexOf(stop) <= -1) {
              this._dag[path[i]][path[i+1]].push(stop);
            }
          }
        } else {
